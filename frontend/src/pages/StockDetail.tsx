/**
 * StockDetail.tsx — 股票详情页 V2.0
 * 改动：显示股票名称、多维度涨跌概率条（T+1/T+3）、自动生成预测、行情信息条
 */
...
type Analysis = {
  code: string; market: string; name: string
  score: {
    total_score: number; fundamental_score: number; technical_score: number
...
function ProbBar({ label, probUp }: { label: string; probUp: number }) {
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      <div className="prob-bar">
        <div className="up-part" style={{ width: `${upPct}%` }}>涨{upPct}%</div>
        <div className="down-part" style={{ width: `${downPct}%` }}>跌{downPct}%</div>
      </div>
    </div>
  )
}
...
export default function StockDetail() {
  ...
  useEffect(() => {
    ...
    // V2.0: 自动触发预测，再加载分析
    marketApi.predict(code, market).catch(() => {}).finally(() => {
  ...
  const klineOpt = {
    ...
    xAxis: [
      { type: 'category' as const, data: data.history_dates, gridIndex: 0,
        axisLabel: { color: '#64748b', fontSize: 11 }, axisLine: { lineStyle: { color: '#252936' } } },
      { type: 'category' as const, data: data.history_dates, gridIndex: 1, axisLabel: { show: false } },
    ],
    yAxis: [
    ...
    series: [
      { type: 'line' as const, data: data.history_close, xAxisIndex: 0, yAxisIndex: 0, symbol: 'none', smooth: true,
        lineStyle: { color: '#818cf8', width: 2 },
        areaStyle: { color: { type: 'linear' as const, x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(99,102,241,.25)' }, { offset: 1, color: 'rgba(99,102,241,0)' }] } } },
      { type: 'bar' as const, data: data.history_volume, xAxisIndex: 1, yAxisIndex: 1,
  ...
  const r = 42, ringCirc = 2 * Math.PI * r
  const ringArc = score.total_score / 100 * ringCirc
    ...
    <div style={{ padding: '26px 30px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <h1 style={{ fontSize: 21, fontWeight: 700 }}>{data.name}（{code}）</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 312px', gap: 20 }}>
                ...
                <tbody>
                  {data.recent_predictions.map((p, i) => (
                    <tr key={i} style={{ borderBottom: i < data.recent_predictions.length - 1 ? '1px solid var(--border)' : 'none' }}>
                      ...
                      <td style={{ padding: '9px 10px', color: dirColor[p.direction], fontWeight: 500 }}>{dirLabel[p.direction] || p.direction}</td>
                      ...
                      <td style={{ padding: '9px 10px' }}>
                        {p.is_correct === null ? <span style={{ color: 'var(--muted)' }}>—</span>
                          : p.is_correct ? <span style={{ color: 'var(--up)' }}>✓ 准确</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 16 }}>
              <svg width={100} height={100} viewBox="0 0 100 100">
                ...
                <circle cx={50} cy={50} r={r} fill="none" stroke={ringColor} strokeWidth={6}
                  strokeLinecap="round" strokeDasharray={`${ringArc.toFixed(1)} ${ringCirc.toFixed(1)}`}
                ...
                <text x={50} y={46} textAnchor="middle" fontSize={22} fontWeight={700}
                  fill={ringColor} fontFamily="DM Mono, monospace">{Math.round(score.total_score)}</text>
          ...
          {/* V2.0: 涨跌预测卡片 */}
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '18px 20px' }}>
            <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 14 }}>涨跌预测</div>
            <ProbBar label="T+1（明日）" probUp={score.prob_up} />
            <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 12, fontFamily: 'monospace' }}>
              预测区间: {score.pred_range_low.toFixed(1)}% ~ {score.pred_range_high.toFixed(1)}%
            </div>
            <ProbBar label="T+3（三日后）" probUp={Math.max(0.3, Math.min(0.7, score.prob_up * 0.95))} />
            ...
            <div style={{
              marginTop: 14, padding: '10px 12px', background: 'var(--bg3)', borderRadius: 8,
